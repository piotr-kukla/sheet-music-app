...
import com.algocode.sheetmusic.email.sender.DummyEmailSender
import com.algocode.sheetmusic.test.*
...
class PasswordResetApiTest extends BaseTest with Eventually with TestDependencies:
  ...
  def codeSentToEmail(email: String): String =
    ...
    val emailData = DummyEmailSender
      .findSentEmail(email, "SoftwareMill Sheetmusic password reset")
  ...
  def codeWasNotSentToEmail(email: String): Unit =
    ...
    val maybeEmail = DummyEmailSender.findSentEmail(email, "SoftwareMill Sheetmusic password reset")
    maybeEmail match
      case Some(emailData) =>
